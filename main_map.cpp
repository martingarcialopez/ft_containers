...
map<T1, T2> mp;
map<T1, T2>::iterator it = mp.end();

void	printPair(map<T1, T2>::iterator ret) {

	std::cout << "pair: " << ret->first << " " << std::endl;

}

void	ft_find(T1 const &k)
{
	map<T1, T2>::iterator ret = mp.find(k);
...
int		main(void)
{
	...
	map<T1, T2> /*const*/ c_map(mp.begin(), mp.end());
